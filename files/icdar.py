...
tf.app.flags.DEFINE_string('training_data_list', '/data/ocr/icdar2015/train_list.txt',
                           'training dataset list to use')
...
def get_images():
    files = []
    im_list = [l.strip() for l in open(FLAGS.training_data_list).readlines()]
    dirname = os.path.dirname(FLAGS.training_data_list)
    files = [os.path.join(dirname, l) for l in im_list]
    files = [f for f in files if f.split('.')[-1] in ['jpg', 'png', 'jpeg', 'JPG']]
    return files
...
def generator(input_size=512, batch_size=32,
    ...
    print('{} training images in {}'.format(
        image_list.shape[0], FLAGS.training_data_list))
        ...
        for i in index:
            try:
                ...
                if im is None:
                    print(im_fn)
                h, w, _ = im.shape
                im_tag = os.path.basename(im_fn).split('.')[0]
                txt_fn = os.path.join(os.path.dirname(FLAGS.training_data_list), 'train_gt_t13', im_tag + '.txt')
